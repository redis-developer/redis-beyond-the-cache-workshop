---
id: WL4
title: Search Workshop Directory Migration
status: blocked
depends_on:
  - WL1
  - WL2
...
Move workshop 2 into the canonical workshop-owned directory layout.

# Why this exists

- Workshop 2 has its own docs, standalone compose, and frontend/backend assets that should move together under one workshop root.

# Required changes

- Move the backend module to `java-springboot/2_full_text_search/backend`.
- Move the frontend module to `java-springboot/2_full_text_search/frontend`.
- Keep workshop-local docs and standalone compose at `java-springboot/2_full_text_search/`.
...
- All workshop 2 backend code lives under `java-springboot/2_full_text_search/backend`.
- All workshop 2 frontend code lives under `java-springboot/2_full_text_search/frontend`.
...
- `/bin/zsh -lc 'env JAVA_HOME=$(/usr/libexec/java_home -v 21) ./java-springboot/gradlew -p java-springboot --no-daemon :2_full_text_search:test --rerun-tasks --tests com.redis.workshop.search.FullTextSearchBackendModeIntegrationTest'`
- `docker compose -f java-springboot/2_full_text_search/docker-compose.yml config --services`
...
- Record the final workshop 2 directory tree.
