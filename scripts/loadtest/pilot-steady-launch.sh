...
ACTOR_ID_PREFIX="${ACTOR_ID_PREFIX:-steady-learner}"
...
mkdir -p "$OUTPUT_DIR"
RUN_ID="steady-$(date -u +%Y%m%dT%H%M%SZ)"
OUTPUT_FILE="$OUTPUT_DIR/$RUN_ID.jsonl"
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/pilot-steady.XXXXXX")"
trap 'rm -rf "$TMP_DIR"' EXIT

echo "Writing steady launch results to $OUTPUT_FILE" >&2

index=1
while [ "$index" -le "$ITERATIONS" ]; do
  workshop_index=$(( (index - 1) % ${#WORKSHOPS[@]} ))
  workshop_id="${WORKSHOPS[$workshop_index]}"
  actor_id="${ACTOR_ID_PREFIX}-${index}"
  create_file="$TMP_DIR/create-$index.json"
  launch_file="$TMP_DIR/launch-$index.json"
  final_file="$TMP_DIR/final-$index.json"
  start_time="$(date +%s)"
  ...
  cleanup_lag_seconds="null"
  termination_requested=false
  launch_end_time="$start_time"

  echo "[$index/$ITERATIONS] launching $workshop_id as $actor_id" >&2
...
echo "Steady launch run complete: $OUTPUT_FILE" >&2
