...
export default {
  name: 'SpringAiFundamentalsEditor',
  components: {
    WorkshopCodeEditorShell,
    WorkshopContentRenderer,
    WorkshopEditorLayout
  ...
  data() {
    return {
      ...
      files: [
        ...
        'SpringAiFundamentalsAiConfig.java',
        'SpringAiDemoService.java',
  ...
  computed: {
    ...
    editorTitle() {
      return this.content?.title || 'Spring AI Fundamentals Editor'
    },
    redisInsightUrl() {
  ...
  methods: {
    handleContentRenderError(issues) {
      console.warn('Spring AI editor content render issues:', issues)
    },
    async loadContent() {
      try {
        this.content = await fetchWorkshopContent(this.editorContentId)
      } catch (error) {
        console.error('Error loading Spring AI editor content:', error)
