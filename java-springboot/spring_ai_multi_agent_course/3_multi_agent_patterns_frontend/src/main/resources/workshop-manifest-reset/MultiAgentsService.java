...
import com.redis.workshop.springai.multiagents.memory.AmsChatMemoryRepository;

@Service
public class MultiAgentsService {

    private static final String MODULE_NAME = "redis-agent-memory";
    ...
    public MultiAgentsRunResponse run(MultiAgentsRunRequest request) {
        ...
        // ), List.of(), result, null, null, null, null, null);
    ...
    private String conversationId(String value) {
        String normalized = nonBlank(value, "redis-agent-memory-demo");
        if (normalized.contains(AmsChatMemoryRepository.SEPARATOR)) {
            return normalized;
        }
        return AmsChatMemoryRepository.createConversationId("workshop-user", normalized);
    }
