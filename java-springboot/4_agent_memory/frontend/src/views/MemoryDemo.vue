...
export default {
  ...
  methods: {
    ...
    async startConversation() {
      this.loading = true;
      try {
        const res = await fetch(`/api/demo/conversation?userId=${encodeURIComponent(this.userId)}`, {
          method: 'POST'
        });
        const data = await res.json();
        if (data.success) {
    ...
    async getConversation() {
      this.loading = true;
      try {
        const res = await fetch(`/api/demo/conversation/${encodeURIComponent(this.sessionId)}`);
        const data = await res.json();
        if (data.success) {
    ...
    async addMessage() {
      ...
      try {
        const res = await fetch(
          `/api/demo/conversation/${encodeURIComponent(this.sessionId)}/message?role=${this.newMessageRole}&content=${encodeURIComponent(this.newMessageContent)}`,
          { method: 'POST' }
        );
        const data = await res.json();
        if (data.success) {
    ...
    async storePreference() {
      ...
      try {
        const res = await fetch(
          `/api/demo/preference?userId=${encodeURIComponent(this.userId)}&preference=${encodeURIComponent(this.newPreference)}`,
          { method: 'POST' }
        );
        const data = await res.json();
    ...
    async searchMemories() {
      ...
      try {
        const res = await fetch(
          `/api/demo/preferences?userId=${encodeURIComponent(this.userId)}&query=${encodeURIComponent(this.searchQuery || 'preferences')}`
        );
        const data = await res.json();
