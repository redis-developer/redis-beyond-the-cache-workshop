...
export default {
  ...
  methods: {
    ...
    async storeWorkingMemory() {
      this.loading = true;
      try {
        const res = await fetch(`/api/demo/conversation?userId=${encodeURIComponent(this.exercise1.userId)}&sessionId=${encodeURIComponent(this.exercise1.sessionId)}`, { method: 'POST' });
        await res.json();
        // Add messages
        await fetch(`/api/demo/conversation/${encodeURIComponent(this.exercise1.sessionId)}/message?role=user&content=${encodeURIComponent(this.exercise1.userMessage)}`, { method: 'POST' });
        await fetch(`/api/demo/conversation/${encodeURIComponent(this.exercise1.sessionId)}/message?role=assistant&content=${encodeURIComponent(this.exercise1.assistantMessage)}`, { method: 'POST' });
    ...
    async retrieveWorkingMemory() {
      this.loading = true;
      try {
        const res = await fetch(`/api/demo/conversation/${encodeURIComponent(this.exercise1.sessionId)}`);
        const data = await res.json();
    ...
    async storeLongTermMemory() {
      this.loading = true;
      try {
        const res = await fetch(`/api/demo/preference?userId=${encodeURIComponent(this.exercise2.userId)}&preference=${encodeURIComponent(this.exercise2.memoryText)}`, { method: 'POST' });
        await res.json();
    ...
    async searchMemories() {
      this.loading = true;
      try {
        const res = await fetch(`/api/demo/preferences?userId=${encodeURIComponent(this.exercise2.userId)}&query=${encodeURIComponent(this.exercise2.searchQuery)}`);
        const data = await res.json();
    ...
    async buildContext() {
      ...
      try {
        // Get working memory
        const wmRes = await fetch(`/api/demo/conversation/${encodeURIComponent(this.exercise1.sessionId)}`);
        const wmData = await wmRes.json();
        // Search for relevant memories
        const ltmRes = await fetch(`/api/demo/preferences?userId=${encodeURIComponent(this.exercise2.userId)}&query=${encodeURIComponent(this.exercise3.newMessage)}`);
