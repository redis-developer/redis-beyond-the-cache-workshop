...
grep -q 'code-server@${CODE_SERVER_VERSION}' "$dockerfile"
grep -q 'WORKSHOP_LOCAL_CODE_EDITOR_COMMAND' "$dockerfile"
grep -q 'WORKSHOP_LOCAL_CODE_EDITOR_PORT=39000' "$dockerfile"
grep -q 'WORKSHOP_LOCAL_CODE_EDITOR_HEALTH_PATH=/healthz' "$dockerfile"
grep -q '127.0.0.1:${editor_port}' "$dockerfile"
grep -q 'WORKSHOP_SESSION_WORKSPACE_PATH' "$dockerfile"
grep -q 'SHELL=/bin/false' "$dockerfile"
